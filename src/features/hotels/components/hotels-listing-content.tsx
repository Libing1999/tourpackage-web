...
import { HotelCard } from "@/features/home/components/hotel-card";
import { useHotelsList } from "../hooks/use-hotels";
import { HotelFilters, type HotelFiltersValue } from "./hotel-filters";
import { HotelSortSelect } from "./hotel-sort-select";
import type { HotelListFilters, HotelSortOption } from "../types";

const PAGE_SIZE = 12;

function parseFilters(params: URLSearchParams): HotelListFilters {
  const amenityIds = params.get("amenities");
  return {
    search: params.get("q") ?? undefined,
    minPrice: params.get("minPrice") ? Number(params.get("minPrice")) : undefined,
    maxPrice: params.get("maxPrice") ? Number(params.get("maxPrice")) : undefined,
    minStarRating: params.get("stars") ? Number(params.get("stars")) : undefined,
    amenityIds: amenityIds ? amenityIds.split(",").filter(Boolean) : [],
    sort: (params.get("sort") as HotelSortOption | null) ?? "recommended",
    page: params.get("page") ? Number(params.get("page")) : 0,
    size: PAGE_SIZE,
  };
}

function toSearchParams(filters: HotelListFilters): URLSearchParams {
  const params = new URLSearchParams();
  if (filters.search) params.set("q", filters.search);
  if (filters.minPrice !== undefined) params.set("minPrice", String(filters.minPrice));
  if (filters.maxPrice !== undefined) params.set("maxPrice", String(filters.maxPrice));
  if (filters.minStarRating !== undefined) params.set("stars", String(filters.minStarRating));
...
export function HotelsListingContent() {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const heading = useBlock("page.hotels");
  ...
  const updateFilters = useCallback(
    (patch: Partial<HotelListFilters>, resetPage = true) => {
      const next: HotelListFilters = {
        ...filters,
        ...patch,
  ...
  const { data, isPending, isError } = useHotelsList(filters);

  const filtersValue: HotelFiltersValue = {
  ...
  const filterPanel = (
    <HotelFilters
      ...
      onClear={() =>
        updateFilters({ minPrice: undefined, maxPrice: undefined, minStarRating: undefined, amenityIds: [] })
      }
    />
  );
        ...
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <div className="mb-6 flex animate-rise flex-col gap-1.5">
            ...
            <p className="text-sm text-muted-foreground">
              {isPending ? "Searching hotels…" : `${data?.totalElements ?? 0} hotels found`}
            </p>
          </div>
              ...
              {isError ? (
                ...
                <>
                  <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
                    {data.content.map((hotel, i) => (
                      <Reveal className="h-full" key={hotel.id} delayMs={Math.min(i, 6) * 60}>
                        <Link href={`/hotels/${hotel.slug}`} className="block h-full rounded-2xl focus-visible:ring-3 focus-visible:ring-ring/50 focus-visible:outline-none">
                ...
                <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed bg-card/60 p-16 text-center">
                  <p className="font-medium text-foreground">No hotels match your filters</p>
